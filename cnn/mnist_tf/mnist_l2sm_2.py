...
def inference(images, isTrain=True, toClassify=True, isBN=True, isUnitNorm = False):
  ...
  scale = tf.get_variable('scale', dtype=tf.float32, initializer=tf.constant(2.0), trainable=False)
  fc4 = tf.multiply(scale, fc4, name='scale_alpha')
  ...
  if toClassify:
      ...
      return softmax_linear, fc4, scale
  else:
      return fc4
