...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 17 16:19:40 2017
...
def train():
  with tf.Graph().as_default():
    ...
    ########################
    # CL ZONE
    logits_id, local, cent_var, kappaVal, wts, t_num_dim = cvn.inference_rec_net_cent_var(images, CODE_LEN, NUM_CLASSES)
    ...
    #scale = tf.get_variable('scale', dtype=tf.float32, initializer=tf.constant(3.16), trainable=False)
    #z_mean = tf.multiply(scale, z_mean, name='kappa_mu')
    
    # Draw new sample
    #samples = tf.random_normal([FLAGS.batch_size,CODE_LEN],0,1,dtype=tf.float32)
    #guessed_z = z_mean + samples
    weight_maps = tf.gather(tf.transpose(wts), labels)
    #guessed_z = local
    # guessed_z = tf.add(cent_var, weight_maps)
    guessed_z = weight_maps*kappaVal
    ...
    opt_rec = tf.train.AdamOptimizer(INITIAL_LEARNING_RATE)
