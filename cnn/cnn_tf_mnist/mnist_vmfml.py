...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 15 22:42:56 2017
...
def inference(images, isTrain=True, toClassify=True, isBN=True, isUnitNorm = False):
  ...
  kappa = tf.get_variable('kappa', dtype=tf.float32, initializer=tf.constant(2.0), trainable=False)
  fc4 = tf.multiply(kappa, fc4, name='kappa_mu')
  ...
  if toClassify:
      inpNumNeurons = 3
      softmax_linear = do_fc_norm('softmax_linear', fc4, inpNumNeurons, NUM_CLASSES, stdVal=0.04, wt_d=0.004)
      return softmax_linear, fc4, kappa
  else:
      return fc4
